...
def evaluate_fight(
    ...
    Bet rules:
      Consensus bet  — model and market agree on winner, MIN_EDGE <= edge <= MAX_EDGE
      Contrarian bet — model disagrees with market, MIN_EDGE * 2 <= edge <= MAX_EDGE
    ...
    threshold = MIN_EDGE if agrees else MIN_EDGE * 2
